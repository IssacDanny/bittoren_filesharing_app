...
class Node:
    ...
    def send_http_request(self, endpoint: str, payload: dict) -> dict:
        """Send an HTTP POST request to the tracker."""
        try:
            response = requests.post(
                f"http://{config.constants.TRACKER_ADDR[0]}:{config.constants.TRACKER_ADDR[1]}{endpoint}",
                json=payload
            )
            response.raise_for_status()
            return response.json()
        except requests.RequestException as e:
            log(self.node_id, f"HTTP request failed: {e}")
            return {}

    def send_https_request_download(self, tracker_url: str, payload: dict) -> dict:
        """Send an HTTP POST request to the tracker that keep record of node which hold the file."""
        try:
            response = requests.post(
                tracker_url,
                json=payload
    ...
    def set_send_mode(self, filename: str):
        port = self.send_port
        file_path = f"{config.directory.node_files_dir}node{self.node_id}/{filename}"
        torrent_dir = f"{config.directory.node_files_dir}node{self.node_id}/"

        file_hash = calculate_file_hash(file_path)
        torrent_path = os.path.join(torrent_dir, f"{file_hash.hex()}.torrent")

        if filename not in self.files:
            log(self.node_id, f"You don't have {filename}")
            return

        # Check if the torrent file exists or not
        if not os.path.isfile(torrent_path):
            # Generate the torrent file if not exists
            create_torrent(file_path, torrent_dir)
        ...
        response = self.send_http_request("/tracker", payload)
        log(self.node_id, f"Send mode set for {filename}: {response}")
    ...
    def set_download_mode(self, torrent_filename: str):
        torrent_file_path = f"{config.directory.node_files_dir}node{self.node_id}/{torrent_filename}"
        if torrent_filename not in self.files:
            log(self.node_id, f"You don't have {torrent_filename}")
            return

        parsed_info = parse_torrent_file(torrent_file_path)
        tracker_url = parsed_info[0]
        filename = parsed_info[1]
        file_path = f"{config.directory.node_files_dir}node{self.node_id}/{filename}"
        ...
        log(node_id=self.node_id, content=log_content)
        # send message for tracker to search for the peers that holds the file
        payload = {
        ...
        response = self.send_https_request_download(tracker_url, payload)

        file_owners = response.get("search_result", [])
    ...
    def exit_torrent(self):
        ...
        response = self.send_http_request("/tracker", payload)
        log(self.node_id, f"Exited torrent: {response}")
    ...
    def enter_torrent(self):
        ...
        response = self.send_http_request("/tracker", payload)
        log(self.node_id, f"Entered torrent: {response}")
    def inform_tracker_periodically(self, interval: int):
        ...
        self.send_http_request("/tracker", payload)
        log(self.node_id, "Informed tracker")
